...
import { createTool, MCPConfiguration } from "@voltagent/core";
import { z } from "zod";
...
export const fetchAndValidateFigmaTool = createTool({
  ...
  execute: async ({ figmaUrl }, context) => {
    ...
    try {
      ...
      console.log(`   - Size: ${JSON.stringify(validationResult).length} chars`);

      // ======================================================================
      // Step 9: Save to file system (persistent backup)
      // ======================================================================

      console.log("💾 Saving to file system...");

      // Ensure workspace/json directory exists
      await fs.mkdir("workspace/json", { recursive: true });

      // Save validated JSON
      const filePath = "workspace/json/validated-figma-data.json";
      await fs.writeFile(
        filePath,
      ...
      console.log(
        `📏 File size: ${JSON.stringify(validationResult).length} characters`
      );
